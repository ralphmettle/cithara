...
def test_canonise_method():
    canonised = Note("Eb")._canonise()
    assert Note("C#")._canonise(use_flats=False) == "C#"
...
def test_generate_from_interval():
    assert (
        NoteGenerator.from_interval(root=Note("C"), interval=Interval(4)).note_name
        == Note("E").note_name
    )
    assert (
        NoteGenerator.from_interval(root=Note("C"), interval=Interval(3)).note_name
        == Note("Eb").note_name
    )
    assert (
        NoteGenerator.from_interval(
            root=Note("C"), interval=Interval(3), use_flats=False
        ).note_name
        == Note("D#").note_name
    )
