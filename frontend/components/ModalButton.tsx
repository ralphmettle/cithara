...
function ModalDisplay({
  ...
  return (
    <div
      onClick={onClose}
      className="fixed flex items-center justify-center inset-0 bg-cithara-bg/50 backdrop-blur-lg w-full h-full cursor-default"
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="relative flex flex-col p-10 items-center justify-center w-md bg-cithara-panel border border-cithara-panel-border shadow-2xl rounded-2xl cursor-default"
      >
        <div className="absolute top-4 left-4">
          <Button onClick={onClose} icon={<IoClose className="w-4 h-4" />} />
        </div>
        {children}
      </div>
    </div>
